...
//     const txResult = await presaleFactoryContract.methods.create(hexSoftCap, hexHardCap, startDate, presaleRate, tokenAddr, ).send({ 'from': account, 'value': 100000000000000000 })
     const txResult = await presaleContract.methods.setWhitelistInfo(whitelistArray).send({'from': account})
  ...
  return (
    <>
    <CButton color="yellow" shape = "rounded-2" style={{backgroundColor: '#000', width: '100%'}} onClick={() => setVisible(!visible)}>Add users to whitelist</CButton>
    <CModal style={{backgroundColor: '#333'}} scrollable visible={visible} onClose={() => setVisible(false)}>
      ...
      <CModalFooter>
        <CButton color="yellow" onClick={handleAddUser} >Add Users</CButton>
      </CModalFooter>
    </CModal>
    </>
  )
